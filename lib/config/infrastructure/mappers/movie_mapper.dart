import 'package:cinemafan/config/constants/environment.dart';
import 'package:cinemafan/config/domain/entities/movie.dart';
import 'package:cinemafan/config/infrastructure/models/moviedb_models.dart';

class MovieMapper {
  static const String _imageWidth = '500';
  static const String _noImagePath = 'data:image/jpeg;base64,';
  static const String _noPosterImagePath = 'data:image/png;base64,';

  static Movie movieFromMovieDbToEntity(MovieFromMovieDb movieFromMovieDb) => Movie(
    adult: movieFromMovieDb.adult,
    backdropPath: movieFromMovieDb.backdropPath == ''
      ? _noImagePath
      : '${Environment.theMovieDbImageBaseUrl}/w$_imageWidth${movieFromMovieDb.backdropPath}',
    genreIds: movieFromMovieDb.genreIds.map((e) => e.toString()).toList(),
    id: movieFromMovieDb.id,
    originalLanguage: movieFromMovieDb.originalLanguage,
    originalTitle: movieFromMovieDb.originalTitle,
    overview: movieFromMovieDb.overview,
    popularity: movieFromMovieDb.popularity,
    posterPath: movieFromMovieDb.posterPath == ''
      ? _noPosterImagePath
      : '${Environment.theMovieDbImageBaseUrl}/w$_imageWidth${movieFromMovieDb.posterPath}',
    releaseDate: movieFromMovieDb.releaseDate,
    title: movieFromMovieDb.title,
    video: movieFromMovieDb.video,
    voteAverage: movieFromMovieDb.voteAverage,
    voteCount: movieFromMovieDb.voteCount,
  );


  static Movie movieDetailsFromMovieDbToEntity(MovieDetailsFromMovieDb movieDetailsMovieDb) => Movie(
    adult: movieDetailsMovieDb.adult,
    backdropPath: movieDetailsMovieDb.backdropPath == ''
      ? _noImagePath
      : '${Environment.theMovieDbImageBaseUrl}/w$_imageWidth${movieDetailsMovieDb.backdropPath}',
    genreIds: movieDetailsMovieDb.genres.map((e) => e.name.toString()).toList(),
    id: movieDetailsMovieDb.id,
    originalLanguage: movieDetailsMovieDb.originalLanguage,
    originalTitle: movieDetailsMovieDb.originalTitle,
    overview: movieDetailsMovieDb.overview,
    popularity: movieDetailsMovieDb.popularity,
    posterPath: movieDetailsMovieDb.posterPath == ''
      ? _noPosterImagePath
      : '${Environment.theMovieDbImageBaseUrl}/w$_imageWidth${movieDetailsMovieDb.posterPath}',
    releaseDate: movieDetailsMovieDb.releaseDate,
    title: movieDetailsMovieDb.title,
    video: movieDetailsMovieDb.video,
    voteAverage: movieDetailsMovieDb.voteAverage,
    voteCount: movieDetailsMovieDb.voteCount,
  );

}
